...
export default function TradeHistoryPage() {
  ...
  const fetchTradeHistory = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getTradeHistory(id as string, symbol, session.accessToken);
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <p className="text-center py-4">No trade history found for {symbol}</p>
              ) : (
                    ...
                    <TableBody>
                      {tradeHistory.map((trade) => (
                        <TableRow key={trade.id}>
                          ...
                          <TableCell>{parseFloat(trade.quoteQty).toFixed(2)}</TableCell>
                          <TableCell>
                            {parseFloat(trade.commission).toFixed(8)} {trade.commissionAsset}
                          </TableCell>
                          <TableCell>{trade.isMaker ? 'MAKER' : 'TAKER'}</TableCell>
                          <TableCell>
                            {trade.realizedPnl ? (
                              <span className={parseFloat(trade.realizedPnl) >= 0 ? 'text-green-600' : 'text-red-600'}>
                                {parseFloat(trade.realizedPnl) >= 0 ? '+' : ''}{parseFloat(trade.realizedPnl).toFixed(2)} USDT
                              </span>
                            ) : '-'}
                          </TableCell>
                        </TableRow>
